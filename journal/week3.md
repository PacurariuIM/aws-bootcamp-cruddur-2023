...
## AWS Amplify
- we'll install the AWS Amplify framework:
```
...
- we put the following code into our `HomeFeedPage.js`:
```
...
- create a file `cognito_jwt_token.py`:
```
import time
import requests
...
class CognitoJwtToken:
  ...
  - in backend-flask/services, in the file `home_activities.py` modify the class HomeActivities:
  ```
 ...
 ```
 - in frontend-react-js/src/pages/ add to the `RecoverPage.js` file the following:
 ```
 import { Auth } from 'aws-amplify';
 ...
 export default function RecoverPage() {
  ...
  ```
  - in `SignupPage.js` replace `username` with `email`:
  ```
  email: email,
  ```
